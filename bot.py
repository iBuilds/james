...
def update_bot():
    ...
    for x in range(len(_hash_buy)):
        ...
        if _hash_check == 'filled' and status_sale[x] != 'unfilled':
            ...
            _response = place_ask(cryptocurrency, coin_buy[x], _rate)
            if _response.json()['error'] == 17 or _response.json()['error'] == 18:
                _rate = float(_rate_buy[x])
                if _coin_sale[x] < amount and _wallet >= amount:
                    _coin_sale[x] = amount
    ...
    for x in range(len(_hash_sale)):
        ...
        if _hash_check == 'filled':
            ...
            _total_profit.pop(0)
            _total_profit = [float(i) for i in _total_profit]
            sale_message(_statement[0], _statement[2], _statement[6], _statement[4], _statement[7], str(round(sum(map(float, _total_profit)), 2)))
            _rate = float(_rate_buy[x])
            if _coin_sale[x] < amount and _wallet >= amount:
                _coin_sale[x] = amount
            _response = place_bid(cryptocurrency, _coin_sale[x], _rate)
