...
def credit_check():
    ...
    signature = sign(data)
    data['sig'] = signature
    response = requests.post(API_HOST + '/api/user/trading-credits', headers=bitkub_header, data=json_encode(data))
    if response.json()['error'] == 0:
        if float(response.json()['result']) <= 10:
            _last_time = datetime.now(tz = tz.gettz("Asia/Bangkok")).strftime("%Y-%m-%d %H:%M:%S")
            message = 'Credit balance\nTime: ' + _last_time + '\nAmount: ' + str(response.json()['result']) + ' THB'
            response = requests.post(LINE_HOST, headers=line_header, data={'message': message})
            print('Credit status message:', response.json())
...
def hash_check(hash):
    ...
    if hash != '':
        ...
        if response.json()['error'] == 0:
            ...
            if response.json()['result']['filled'] == response.json()['result']['amount']:
                queue_data['status'] = 'filled'
                return 'filled'
            else:
                queue_data['status'] = 'unfilled'
                return 'unfilled'
        else:
    ...
    for x in range(len(_hash_buy)):
        ...
        if _hash_check == 'filled' and status_sale[x] != 'unfilled' and amout_crypto_check() >= float(coin_buy[x]):
            ...
            if _response.json()['error'] == 17 or _response.json()['error'] == 18:
                ...
                if float(_coin_sale[x]) > amount or (float(_coin_sale[x]) + _wallet) >= float(_coin_sale[x]):
                    _coin_sale[x] = amount
    ...
    for x in range(len(_hash_sale)):
        ...
        if _hash_check == 'filled':
            ...
            _statement = [
                _last_time,
                currency,
                str(round(float(sheet.row_values(x + 2)[0]) + grid, 2)),
            ...
            if queue_data['status'] == 'unfilled':
                while 1:
                    if _books['asks'][_queue][3] == float(queue_data['rate']) and round(_books['asks'][_queue][4], 2) == round(float(queue_data['amount']), 2):
                        break
                    else:
                        _queue += 1
                queue_sale[x] = _queue
...
# print(place_bid(currency, '69', '0.57').json()['result'])
start_bot()
while 1:
    try:
        credit_check()
