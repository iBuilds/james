...
def amout_crypto_check():
    ...
    response = requests.post(API_HOST + '/api/market/wallet', headers=bitkub_header, data=json_encode(data))
    if response.json()['error'] == 0:
        return response.json()['result'][cryptocurrency]
    else:
        return 0
...
def update_bot():
    ...
    for x in range(len(_hash_buy)):
        ...
        if _hash_check == 'filled' and status_sale[x] != 'unfilled' and amout_crypto_check() >= coin_buy[x]:
    ...
    for x in range(len(_hash_sale)):
        ...
        if _hash_check == 'filled':
            ...
            _statement = [
                ...
                float(sheet.row_values(x + 2)[11]) - float(sheet.row_values(x + 2)[4]) - float(sheet.row_values(x + 2)[3])
            ]
